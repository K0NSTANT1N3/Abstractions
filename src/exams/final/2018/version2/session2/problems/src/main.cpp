...
void readGridFromFile(const string &fileName, Grid<int> &grid, int &answ) {
    ...
    int n, m;
    if (!(file >> n >> m >> answ)) {
        cerr << "Error reading dimensions or answer from file." << endl;
        return;
    }
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < m; ++j) {
            if (!(file >> grid[i][j])) {
                cerr << "Error reading grid value at (" << i << ", " << j << ")" << endl;
                return;
            }
        }
    }
    ...
    file.close();
//    printGrid(grid);  // Assuming printGrid function exists and works correctly
}
...
void runShortestPathTests() {
    ...
    Grid<int> table;
    int answ;
    string path = "/home/konstantine/CLionProjects/Abstractions/src/exams/final/2018/problems/path/test";

    for (int i=2; i<=num; i++) {
        readGridFromFile(path + integerToString(i) + ".txt", table, answ);
